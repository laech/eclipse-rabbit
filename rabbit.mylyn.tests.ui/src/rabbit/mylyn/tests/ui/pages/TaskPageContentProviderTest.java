...
import rabbit.mylyn.internal.ui.pages.TaskPageContentProvider;
...
public class TaskPageContentProviderTest {
  ...
  public void testGetChildren() throws Exception {
    ...
    TaskResource taskFile = new TaskResource(task, root.getProject("p")
        .getFolder("f").getFile("ff"));
    getFileToValueField(page).put(taskFile, 1000L);

    // Link a folder with the file:
    TaskResource taskFolder = new TaskResource(task, taskFile.resource
        .getParent());
    Set<TaskResource> resources = new HashSet<TaskResource>();
    resources.add(taskFile);
    getFolderToFilesField(page).put(taskFolder, resources);

    // Link a project with the folder:
    TaskResource taskProject = new TaskResource(task, taskFolder.resource
        .getParent());
    resources = new HashSet<TaskResource>();
    resources.add(taskFolder);
    getProjectToResourcesField(page).put(taskProject, resources);
    ...
    getFileToValueField(page).put(subtaskFile, 10L);

    TaskResource subtaskFolder = new TaskResource(subTask, taskFolder.resource);
    resources = new HashSet<TaskResource>();
    resources.add(subtaskFile);
    getFolderToFilesField(page).put(subtaskFolder, resources);

    TaskResource subtaskProject = new TaskResource(subTask,
        taskProject.resource);
    resources = new HashSet<TaskResource>();
    resources.add(subtaskFolder);
    getProjectToResourcesField(page).put(subtaskProject, resources);

    resources = new HashSet<TaskResource>();
    resources.add(subtaskProject);
    getTaskToProjectsField(page).put(subTask, resources);
  ...
  public void testHasChildren() throws Exception {
    ...
    // Add task to Mylyn:
    AbstractTaskContainer category = TasksUiPlugin.getTaskList()
    ...
    assertFalse(provider.hasChildren(taskFile));
    {
      ...
      getFileToValueField(page).put(taskFile, 10L);
