...
public class CommandDataAccessor extends AbstractXmlAccessor<CommandEventType, CommandEventListType> {
	...
	@Override protected Collection<CommandEventListType> getCategories(EventListType doc) {
		return doc.getCommandEvents();
	}

	@Override protected String getId(CommandEventType e) {
		return e.getCommandId();
	}

	@Override protected long getUsage(CommandEventType e) {
		return e.getCount();
	}

	@Override protected Collection<CommandEventType> getXmlTypes(CommandEventListType list) {
