...
public class FileEventStorer extends
	...
	@Override protected List<FileEventListType> getXmlTypeCategories(EventListType events) {
		return events.getFileEvents();
	}

	@Override protected boolean hasSameId(FileEventType x, FileEvent e) {
		return x.getFilePath().equals(e.getFile().getFullPath().toString());
	}

	@Override protected boolean hasSameId(FileEventType x1, FileEventType x2) {
		return x1.getFilePath().equals(x2.getFilePath());
	}

	@Override protected void merge(FileEventType main, FileEvent e) {
		main.setDuration(main.getDuration() + e.getDuration());
	}

	@Override protected void merge(FileEventType main, FileEventType x) {
		main.setDuration(main.getDuration() + x.getDuration());
	}

	@Override protected void merge(FileEventListType main, FileEvent e) {
		merge(main.getFileEvent(), e);
	}

	@Override protected void merge(FileEventListType main, FileEventListType data) {
		merge(main.getFileEvent(), data.getFileEvent());
	}

	@Override protected FileEventType newXmlType(FileEvent e) {
	...
	@Override protected FileEventListType newXmlTypeHolder(XMLGregorianCalendar date) {
